...
def prepare_training():
    if config.get('resume') is not None:
        ...
        model_G = models_ours.make(sv_file['model_G'], load_sd=True).cuda()
...
def train(train_loader, model_G, optimizer_G):
    ...
    loss_fn = nn.L1Loss()
    loss_region = utils.Loss_l1_region()
    loss_0 = utils.Averager()
    loss_1 = utils.Averager()
    loss_2 = utils.Averager()

    for batch in tqdm(train_loader, leave=False, desc='train'):
        for k, v in batch.items():
            batch[k] = v.cuda().float()
        src_img = batch['src'].cuda()
        tgt_img = batch['tgt'].cuda()
        seq_src = batch['seq_src'].cuda()
        seq_tgt = batch['seq_tgt'].cuda()
        pre_src_tgt, pre_tgt_src, content_src_tgt, content_tgt_src, feat_0_src_tgt, content_tgt_src_1, feat_0_tgt_src, content_src_tgt_1 = model_G(src_img, tgt_img, seq_src, seq_tgt)
        loss_src = loss_fn(pre_src_tgt, tgt_img)
        loss_tgt = loss_fn(pre_tgt_src, src_img)
        loss_align = loss_fn(content_src_tgt, content_tgt_src)

        loss_G = loss_src + loss_tgt + 0.1*loss_align
        loss_0.add(loss_src.item())
        loss_1.add(loss_tgt.item())
        loss_2.add(loss_align.item())
        
        optimizer_G.zero_grad()
        loss_G.backward()
        optimizer_G.step()
            
    return loss_0.item(), loss_1.item(), loss_2.item()
...
def main(config_, save_path):
    ...
    for epoch in range(epoch_start, epoch_max + 1):
        ...
        if (epoch_val is not None) and (epoch % epoch_val == 0):
            val_res0, val_res1, pre_src_tgt, pre_tgt_src, src_gt, tgt_gt = utils.eval_psnr(val_loader, model_G_)
            
            utils.write_valimg(pre_src_tgt.cpu().numpy(), os.path.join(save_path, 'src_tgt-{}.nii.gz'.format(epoch)))
            utils.write_valimg(pre_tgt_src.cpu().numpy(), os.path.join(save_path, 'tgt_src-{}.nii.gz'.format(epoch)))
            utils.write_valimg(src_gt.cpu().numpy(), os.path.join(save_path, 'src_gt-{}.nii.gz'.format(epoch)))
            utils.write_valimg(tgt_gt.cpu().numpy(), os.path.join(save_path, 'tgt_gt-{}.nii.gz'.format(epoch)))
